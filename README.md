...
| `.removebg` | Reply foto untuk hapus background. Khusus owner bot (butuh `REMOVEBG_API_KEY`) |
...
| `.ai` | Balasan tetap |
| `.getpp` | Ambil foto profil orang lain (tag, reply, atau nomor) |
| `.tiktok <link>` | Download video TikTok tanpa watermark |
| `.tomp3` | Ambil suara dari video (caption `.tomp3` atau reply video) |
| `.toimg` | Ubah sticker jadi foto (reply sticker) |
| `.tagall [pesan]` | Tag semua member grup, dikirim per 100 member |
| `.setnamegroup <nama>` | Ubah nama grup |
| `.setdesk <teks>` | Ubah deskripsi grup |
| `.open` / `.close` | Buka atau tutup grup (semua member / hanya admin yang boleh kirim pesan) |
| `.revoke` | Reset link undangan grup |
| `.antilink on/off` | Non-admin yang kirim link, pesannya dihapus. Admin boleh |
| `.adminonly on/off` | Bot hanya merespons admin di grup itu |
| `.balikinjf` | Prank. Boleh sambil tag atau reply seseorang. Lagu dikirim sebagai link streaming resmi |

Aturan akses:

- **Owner bot** adalah akun WhatsApp yang menautkan bot (pesan dari akun itu sendiri).
- `.removebg`, `.selfmode`, `.self`, `.public`, `.kickall`, `.setwelcome`, `.setleave`, `.welcome`, `.leave`
  hanya bisa dipakai owner bot. Pengaturannya berlaku untuk seluruh bot itu, dan `.removebg` memakai kuota API key.
- `.tagall`, `.setnamegroup`, `.setdesk`, `.open`, `.close`, `.revoke`, `.antilink`, `.adminonly`
  hanya untuk admin grup (owner bot juga boleh). Selain `.tagall`, bot juga harus jadi admin grup.
  `.antilink` dan `.adminonly` disimpan per grup di `groups.json`.
...
alyz-bot/
  ...
  src/
    ...
    lib/ffmpeg.js       konversi media lewat ffmpeg
    lib/groups.js       pengaturan per grup, cek admin, antilink
...
- `.tiktok` memakai layanan pihak ketiga (tikwm.com) yang bisa berubah atau dibatasi sewaktu-waktu.
  Kalau berhenti bekerja, ganti `API_URL` di `src/features/tiktok.js`. Postingan foto (slideshow) belum didukung.
- `.tomp3` dan sticker video memakai `ffmpeg` lewat `src/lib/ffmpeg.js`.
- `.antilink` hanya bisa menghapus pesan kalau bot admin grup.
